...
from models.nm_conv import set_conv_model_origin, update_conv_inter_bpmask
from models.nm_linear import set_linear_model_origin, update_linear_inter_bpmask

def train_one_epoch(
    model: torch.nn.Module,
    train_loader : Iterable,
    criterion, optimizer, epoch, logger, log_freq, use_closure, update=False,
    implicit_sam=False, args=None,
):
    model.train()
    ...
    for batch_idx, (images, targets) in enumerate(train_loader):
        ...
        if update and implicit_sam:
            if args.culinear:
                update_linear_inter_bpmask(
                    model, 
                    train_loader.dataset, 
                    args.num_samples,
                    optimizer.param_groups,
                    args.pattern,
                    args.n_structured, args.m_structured, args.sparsity
                )
            elif args.samconv:
                update_inter_bpmask(
                    model, 
        ...
        if implicit_sam: 
            if args.culinear:
                set_linear_model_origin(model, False)
            elif args.samconv:
                set_conv_model_origin(model, False)
            else:
                raise RuntimeError
        ...
        if implicit_sam: 
            if args.culinear:
                set_linear_model_origin(model, True)
            elif args.samconv:
                set_conv_model_origin(model, True)
            else:
                raise RuntimeError
        
        if use_closure: 
            if implicit_sam: assert isinstance(optimizer, SAM)
            optimizer.step(
                closure, 
                epoch=epoch,
        ...
        _memory.update_meter('train_loss', loss1.item(), n=batch_num)
